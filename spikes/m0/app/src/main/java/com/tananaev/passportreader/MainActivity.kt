    ...
    private inner class ReadTask(private val isoDep: IsoDep, private val bacKey: BACKeySpec) : AsyncTask<Void?, Void?, Exception?>() {
        ...
        override fun doInBackground(vararg params: Void?): Exception? {
            try {
                ...
                Log.i(TAG, "M0 stage: tag discovered, opening card service")
                ...
                service.open()
                Log.i(TAG, "M0 stage: passport applet service opened")
                ...
                try {
                    ...
                    // EF.CardAccess absent (SW 6A82) means the document does not
                    // offer PACE at all. That is a finding, not an error.
                    Log.i(TAG, "M0 stage: PACE unavailable (${e.javaClass.simpleName}: ${e.message})")
                }
                Log.i(TAG, "M0 stage: PACE ${if (paceSucceeded) "SUCCEEDED" else "not used"}")
                service.sendSelectApplet(paceSucceeded)
                Log.i(TAG, "M0 stage: applet selected")
                if (!paceSucceeded) {
                    try {
                        service.getInputStream(PassportService.EF_COM).read()
                        Log.i(TAG, "M0 stage: EF.COM readable without BAC")
                    } catch (e: Exception) {
                        Log.i(TAG, "M0 stage: attempting BAC")
                        service.doBAC(bacKey)
                        Log.i(TAG, "M0 stage: BAC SUCCEEDED")
                    }
                }
                ...
                Log.i(TAG, "M0 stage: CA probed (succeeded=$chipAuthSucceeded)")
                ...
                Log.i(TAG, "M0 stage: AA probed (succeeded=$activeAuthSucceeded)")
                runM0Report()

                // DG2 (facial image) is deliberately NOT read in M0: we never need it,
                // it is the slow read, and it is the most sensitive object on the chip.
            } catch (e: Exception) {
                Log.e(TAG, "M0 READ FAILED — see the last 'M0 stage' line above for how far it got", e)
                return e
            }
            return null
        }
        ...
        /** Upstream implementation, kept unused for reference. See M0Probe.passiveAuth. */
        @Suppress("unused")
        private fun doPassiveAuthUpstream() {
            try {
                ...
                if (dg1Hash.contentEquals(dataHashes[1])
        ...
        /**
         * PRD v1.5 M0 row: inventory, timings, candidates, and the two planted
         * negatives. Everything printed here is PII-free by construction —
         * field names, counts, hashes and verdicts only.
         */
        private fun runM0Report() {
            ...
            // ---- planted negative (i): one flipped byte in DG1 ----
            ...
            // ---- planted negative (ii): issuing CSCA removed from the list ----
            ...
            if (ds == null) {
                ...
                val stripped = M0Probe.loadMasterList(assets.open("masterList"), excludeAnchorFor = ds)
                val withoutCsca = M0Probe.passiveAuth(dg1File, sodFile, stripped)
                // Two independent conditions. The exclusion must actually have
                // removed something, AND the verdict must stop being `allowed:true`.
                // Without the first check, an exclusion that matched nothing looks
