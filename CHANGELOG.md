...
- **Docs.** `docs/product/customer-guide.md` §7.1, `apps/scanner/README.md`,
  `docs/wiki/milestones.md` §6.6 item 7 status: per-operator release-signing
  keystore recipe (PRD §6.6 item 7 part (a), D80/D81) — EC P-256 PKCS12
  keystore via `keytool`, stored under gitignored `secrets/`, fed to
  `assembleRegularRelease` only via the existing `KEYSTORE_FILE`/
  `KEYSTORE_PASSWORD`/`KEY_ALIAS`/`KEY_PASSWORD` env vars, verified
  independently with `apksigner`. Part (b), the owner's own showcase
  keystore, is not generated by this change.
- **Docs.** `docs/logs/M3-KEYSTORE-EVIDENCE-2026-09-06.md` (new),
  `docs/product/customer-guide.md` §7.1 (agent-shell `keytool` gotcha,
  digest-format note), `docs/wiki/milestones.md` §6.6 item 7 status —
  PRD §6.6 item 7 part (b) DONE: owner's showcase keystore generated
  (`secrets/zkagent-showcase.p12`, EC P-256, gitignored), first **signed**
  release APK built (`assembleRegularRelease --offline`, exit 0), signature
  and digest (`1f6bceae0ffe9c2b326f2aab2202f0bdf3df7e5bdd8fac50aba2e1d318407264`)
  independently verified with `apksigner`. D80's release-evidence MUST is
  now a standing obligation on `/release` — no dedicated release-checklist
  doc exists in this repo to also record it in, so it stands recorded in
