...
test('matrix 1: first spend is fresh, replay of the same nonce is refused', async () => {
  const challenge = issueChallenge({ tier: 'B', threshold: 18, ttlMs: 60_000, secret: SECRET, now: 1000 });
...
test('matrix 2: challenge expired at now, valid one ms earlier', () => {
  const challenge = issueChallenge({ tier: 'B', threshold: 18, ttlMs: 500, secret: SECRET, now: 1000 });
  assert.equal(challenge.expires_at, 1500);

  const expired = verifyChallenge(challenge, { now: 1501, secret: SECRET });
  assert.equal(expired.ok, true);
  assert.equal(expired.valid, false);
  assert.equal(expired.reason, 'challenge_expired');

  const stillValid = verifyChallenge(challenge, { now: 1500, secret: SECRET });
...
test('matrix 3: tier C signed by an unlisted key is refused; listed key with maxTier C is valid', () => {
  ...
  const challenge = issueChallenge({
    tier: 'C', threshold: 18, ttlMs: 60_000, secret: SECRET, now: 1000,
    issuer: { privateKey, key_id: 'issuer-1' },
  });

  const unknown = verifyChallenge(challenge, { now: 1000, secret: SECRET, trustedChallengeIssuers: [] });
...
test('matrix 4: unsigned tier C is refused, unsigned tier B is valid', () => {
  const tierC = issueChallenge({ tier: 'C', threshold: 18, ttlMs: 60_000, secret: SECRET, now: 1000 });
  const cResult = verifyChallenge(tierC, { now: 1000, secret: SECRET });
  assert.equal(cResult.valid, false);
  assert.equal(cResult.reason, 'signature_required_at_tier_c');

  const tierB = issueChallenge({ tier: 'B', threshold: 18, ttlMs: 60_000, secret: SECRET, now: 1000 });
  const bResult = verifyChallenge(tierB, { now: 1000, secret: SECRET });
...
test('extra: a nonce that never came from mintNonce is refused as forged', () => {
  ...
  const result = verifyChallenge(forged, { now: 1000, secret: SECRET });
...
test('extra: editing issued_at after minting is caught as a forged nonce (mismatched encoding)', () => {
  ...
  const result = verifyChallenge(tampered, { now: 1000, secret: SECRET });
  assert.equal(result.valid, false);
  assert.equal(result.reason, 'nonce_forged');
});
...
test('extra: every malformed input to verifyChallenge/spendNonce is a real no, never a throw', async () => {
  ...
  for (const bad of malformed) {
    assert.doesNotThrow(() => verifyChallenge(bad, { now: 1000, secret: SECRET }));
    const vResult = verifyChallenge(bad, { now: 1000, secret: SECRET });
