...
Repo-specific: `.claude/remember/` is tracked here by design (LIBRARY_CONVENTIONS §7 exemption), except `.claude/remember/last-review.md` and `fix-ledger.md`, which are gitignored since tracking them made `/branch-review` invalidate itself (writing them dirties the tree pre-Phase-0; committing them moves HEAD past the recorded SHA, failing Phase 0.5). `.claude/stash/` is gitignored for new files (session handover notes, not deliverables); stash files already tracked before that change stay tracked.
