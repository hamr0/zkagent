...
## 17. mdoc — what the EU actually built, and why we didn't (added 2026-08-31)

*Added after archiving — this doc is the standing answer to the recurring "why not do it the EU's way" question, and §15 already brushed against mdoc without saying what one is. Recorded here once, grounded in the M2 capture (`docs/logs/M2-CAPTURE.md`, Finding 1), so it does not have to be re-derived.*

**What an mdoc is.** ISO/IEC 18013-5's mobile-document format — the container behind the EU wallet credentials 8een consumes, and the exact thing the EU AV Blueprint presents (Finding 1: an `mso_mdoc` of doctype `eu.europa.ec.av.1`, carrying one boolean `age_over_18`, presented over OpenID4VP). Structurally: the credential's data elements are CBOR values, each salted and hashed, and the digests are collected into a **Mobile Security Object (MSO)** that the *issuer* signs; the MSO is also bound to a key held in the holder's device. Selective disclosure is then mechanical — reveal an element plus its salt, and the verifier checks it against the signed digest without seeing any sibling element. Elegant, boring, standardised.

**What an mdoc is not: inside a passport.** The chip in a passport or ID card speaks ICAO 9303 — DG1 and a SOD, the format M0 read. There is no mdoc on the chip and no way for a citizen to extract one. An mdoc exists only because an **issuer minted it** into a wallet app after an enrolment ceremony — an eID login, a bank identification, or, notably, a passport NFC read *used as the issuer's input*. The Blueprint's expected flow (Finding 1's sources): install the AV app → enrol once with an issuer → receive the `eu.europa.ec.av.1` mdoc → present it to sites via OpenID4VP. The passport read, where it appears at all, happens once, at enrolment, feeding the issuer — not per verification, feeding the site.

**Its linkability defect.** The MSO signature is byte-identical across presentations — every site the holder presents the same mdoc to sees the same issuer-signed blob, a stable cross-site identifier. This is the same failure class as our risk #8 attestation chains (§5): a fresh envelope per presentation, the same stamp on every one. The Blueprint's mitigation is **batch issuance** — the issuer pre-mints a stack of single-use attestations so no two sites see the same signature — with real ZK deferred to later work. Which is to say: the linkability hole is patched by asking the issuer to do *more* issuing, not less.

**Why zkagent didn't take this road.** Look at what the issuer layer actually consists of — issuer PKI, an enrolment ceremony, a credential supply chain (batch re-issuance included), device binding — and then at what it proves: that the holder's document was genuine *at enrolment time*. The passport chip's own government signature already proves exactly that, directly, at every single read. The entire mdoc apparatus, for this use case, re-asserts a fact the document already carries. zkagent's bet (D24, and NO-GO #3 before it) is to delete the issuer and verify the government's signature at the point of use — and to pay for that honestly, in captcha-grade claims (D27/D29) rather than in issuance infrastructure. mdoc's machinery is why the owner judged it "weird" for this problem: it is issuance-centric, and our problem is verification-centric. The M2 verifier still has to speak to the world the Blueprint is building — that is Finding 1's point and the test verifier's shape — but nothing in that world requires us to *become* an issuer to answer one boolean.

## 18. sig-ed25519/1 — the vouch, plainly (added 2026-08-31)

*Added after archiving — D30 (PRD v1.15) made `sig-ed25519/1` the default mode-B evidence, and the recurring question "what does that signature actually prove?" deserves the same standing answer the rest of this doc gives. Recorded here once, grounded in the PRD (D29, D30, FR12), so it does not have to be re-derived.*

**The two-part picture.** Every presentation has two parts, answering two different questions. Part one is reading the chip: the phone pulls DG1 and the SOD, and the government's signature already sits inside the SOD — verifying it (passive authentication) proves the data came from a genuine issuance. That part needs no attester at all, because the government pre-signed at issuance; the proof of authenticity ships with the document. Part two is the evidence slot (D24), and it answers a different question entirely: why should the *website* believe the phone did part one honestly, when the website never touched the chip? Part one is about the document; part two is about the reader.

**The two ways to answer it.** The math answer (the zk-passport/longfellow class): the circuit re-runs the government signature-chain verification *inside* the proof — it proves possession of genuinely-signed chip data implying the age, without revealing the data. Note carefully what it verifies: the DATA's signature, not chip authenticity — it cannot tell an original chip from a cloned dump (that axis is D29's, below). The trust answer (`sig-ed25519/1`, D30): no math about the passport at all. An attester signs "I vouch for this exact presentation", and the requester pins that attester's public key. Trust is exactly as wide as the pinned key list — one key = one trusted attester (in a mandated-AV world, typically the provider the government designates); more attesters = more pinned keys (FR12: linkability class 'signer', tier ceiling B, since a signer key stable across sites would break tier A's cross-site bar).

**The vouch is honest about its hole.** Where there is no chip authentication — mode A never uses it; and per M0 some documents, e.g. a US passport, simply have none — the signature is *just a vouch*: the attester attests it believes the reading, it does not prove the chip was the original. That hole is known and STATED, not hidden — the same accept-and-state posture D29 takes on clone-replay (mode B accepts non-chip-auth documents, reports `chip_auth` in the verdict, and says so in every claim). Captcha-grade honesty over implied guarantees.

**Why the signature binds the claim, not just the nonce.** If the vouch covered only nonce+scope, the identical signature could be re-attached to a different claim — a different threshold, a different statement — under the same nonce, and the attester would be vouching for a statement it never saw. Binding the claim hash makes the vouch say "I vouch for THIS exact statement". chiproof's plug contract refuses claim-unbound evidence for exactly this replay reason (D24: an evidence type MUST bind the nonce, the claim and the scope, or it is rejected at registration). The concrete byte layout remains PROPOSED (`spikes/m2-handoff/README.md`) pending owner confirmation.
