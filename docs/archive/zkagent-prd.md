# zkagent — PRD v1.45 — 2026-09-02, owner-approved
...
**Version history**: §15. This revision (v1.46 — 2026-09-02, pending owner approval) is a DOCS reconciliation on `chore/memory-consolidation`: findings #6 and #8 marked FIXED in `.claude/remember/findings.md`, the `fix-ledger.md` doc-drift bullet cleared; #10/#11 unchanged, freeze D57 still in force per D60; no device attached this session, so verification debt is unchanged; the new Snackbar wording is pending owner approval. Detail: `.claude/remember/findings.md`, `CHANGELOG.md` [Unreleased].

The prior revision (v1.45 — 2026-09-02, owner-approved) recorded **D60 — the branch close-out.** The owner decided to pause and close this branch with the D57 freeze CARRIED FORWARD, not lifted and not abandoned: "we will exit here to close this branch... we can move to release and future freeze still there, we will take it with next module first thing, a continuation" / "i want to pause this and move on, otherwise its a perpetual delay." The stated reason — stopping an open-ended fix-then-re-review chain from delaying the branch indefinitely — is the justification for closing with the freeze still in force. **Exit-criteria status at close**: (1) and (2) MET (13 fenced sites as of `72e0b2c`); (3) NOT MET — findings #10/#11 remain OPEN at consequence HIGH, the sole reason the freeze does not lift. **This is a deferral, not a lift** — no new §6.2 item/enhancement/UX change lands while D57 stands, Q43-Q48 stay deferred, and clearing criterion (3) is now the FIRST work item of the next module. D60 also carries a verified-against-source PENDING-AT-CLOSE list (blocking findings, open-not-blocking findings, Q38's answered-not-decided status, the deferred UI questions, verification debt on the `72e0b2c` BiometricPrompt fence and the never-device-tested QR-scan/manual-paste path, and one open `.claude/remember/fix-ledger.md` doc-drift bullet). No new questions opened, nothing closed, no renumbering, D57's own row not annotated as lifted. §10 (D60 added), §15 annotated. See below for the prior revision's own summary.

The prior revision (v1.44 — 2026-09-02, owner-approved) was a **correction** to v1.43's finding #5 record, following a `/branch-review` pass that found a real gap in that pass's completeness claim: `BiometricPrompt.AuthenticationCallback` was missed because v1.43's "11 sites" enumeration was syntactic (`runOnUiThread` text matches), not by the actual hazard predicate. Fixed in commit `72e0b2c` — 13 fenced sites, not 11. Stated precisely: D57's exit criterion (2) was NOT actually met between commits `b8e0e05` and `72e0b2c`; it IS met now, from `72e0b2c` onward. The freeze still did NOT lift — criterion (3) remained unmet. No device evidence existed for this gap or its fix, code/bytecode-verified only. Full record in `.claude/remember/findings.md` #5's appended status-change note and `docs/logs/M2-FENCE-EVIDENCE.md`'s follow-up section. §10 (D57 annotated in place), §15 annotated. No new questions opened, no decisions invented, no renumbering.

The prior revision (v1.43 — 2026-09-02, owner-approved) recorded **`.claude/remember/findings.md`
#5 (zero async-cancellation discipline) FIXED, device-proven** — a `LifecycleFence`
(per-Activity-instance, retired in a new `onDestroy`, 11 sites fenced, later corrected to 13 above)
closed D57's exit criterion (2). **The freeze did NOT lift** — criterion (3) was not met, findings
#10/#11 remained open-but-mitigated. Device evidence (Pixel 6a, six tests) also answered Q38 (log
lifetime: survives recreation, not process death) and opened two new questions, Q47 (input-focus
steal corrupting MRZ entry) and Q48 (three indistinguishable reader apps on the launcher — a UX, not
security, gap). New finding #16 (mint-report loss after Activity destruction) is OPEN, unfixed by
decision — a disclosure/product question, not a code defect. Full evidence in
`docs/logs/M2-FENCE-EVIDENCE.md`. §10 (D57 annotated), §11 (Q38 annotated, Q47/Q48 opened), §15
annotated.

The prior revision (v1.42 — 2026-09-02, owner-approved) recorded **D58 step 4
(re-derived Session boundary — `SessionDisplay` projections) LANDED IN PART** in commit `c38833d` —
**all four D58 steps have now landed.** `SessionDisplay.render(lockedMode, handoffState)` becomes the
sole deriver of `modeStatusView.text`/`handoffStatus.text`/`lockButton.isEnabled`/`.text`, verified as
the sole writer (`MainActivity.kt:430-433`), closing `.claude/remember/findings.md` #9 and #14 by
construction and #15 by extending the same admission gate to the QR-scan/manual-paste path. Locked
state takes unconditional precedence, so `HandoffAdmission` (findings #10/#11) no longer needs to stay
for display-correctness reasons — it was KEPT anyway, by RECOMMENDATION only, its remaining value
narrowed to preventing field overwrite of `pendingHandoff`/`verifiedRequest` while locked or reading.
**Job 4's own deliverable, the re-derived Session boundary, was reported — deliberately NOT
implemented**: the coder recommends two units (a session-state holder plus `SessionDisplay` as a
separate stateless projection), not the single `SessionState` the structure proposal proposed. A
behaviour change (`wipeSession` no longer force-enables the Lock button) was flagged and verified not
to strand a user. Device-confirmed on the Pixel 6a (pid 21642): two mode-B mints, plus a
consumed-session scan correctly running as a local read (finding #14's exact scenario, confirmed not to
recur) — but the on-screen `handoffStatus.text` line itself was NOT visually re-verified this session.
**Three new owner UI items opened as Q44/Q45/Q46** (§11): dimming a completed run with ticked
checkboxes, a single control distinguishing "verify" vs. "scan local," and a label-correctness defect
("Passport number" should read "Document number"/"Passport/ID") — all deferred under D57 alongside
Q43. **Q40's "Tap and scan" Lock label shipped in `c38833d`, noted PROVISIONAL pending owner
approval.** With #9/#13/#14/#15 closed and #10/#11 mitigated, finding #5 (zero async-cancellation
discipline) is now the PRINCIPAL REMAINING BLOCKER on D57's exit criterion. Full evidence in
`docs/logs/M2-D58-STEP4-EVIDENCE.md`. §10 (D58 step-4 status line), §11 (Q40 annotated, Q44/Q45/Q46
opened), §15 annotated.

The prior revision (v1.41 — 2026-09-02, owner-approved) records **D58 step 3
(lock-time snapshot) LANDED** in commit `65096b9` — an immutable `AuthorizedHandoff` is captured once
at lock time and threaded through the read/mint pipeline as a parameter, closing
`.claude/remember/findings.md` #2/#3 by construction and meeting D58's verbatim condition that the
snapshot must land. **`HandoffAdmission` was KEPT, not removed, contrary to D58's stated
expectation** — a separate, still-live projection defect (`applyHandoffVerificationOutcome` not
respecting `lockedMode`) blocks removal until D58 step 4 fixes it. A pre-existing, same-class gap was
found by this step's own required survey and recorded as `.claude/remember/findings.md` #15: the
QR-scan/manual-paste handoff path (`applyPendingHandoffText`) has no admission gate at all, unlike the
`av://` intent path. **D59's owner-approved `ReportLog.MAX_ENTRIES = 20` also LANDED**, in commit
`ff15629`, replacing the provisional `200` from D58 step 1 — closing findings.md #13 as FIXED, no
longer provisional. §10 (D58's status line updated, D59's status line updated), §15 annotated.

The prior revision (v1.40 — 2026-09-02, owner-approved) records two further
owner decisions from 2026-09-02. **D59** clarifies D58 step 1's provisional
`ReportLog.MAX_ENTRIES` constant (commit `c856f42`): the cap counts ENTRIES, not lines — one entry
is a whole scan-outcome block of roughly 20 rendered lines — and the owner-approved value is **20**,
oldest-first eviction unchanged; 200 entries would be an unusable scroll. The code change itself was
**NOT yet applied** at that revision — it was a one-constant change queued behind D58 step 3, then in
flight, to avoid two agents building the same module at once (it has since landed — see above).
**Q43** opens a UI/UX enhancement, DEFERRED under D57's freeze and not designed here:
collapsible log entries, collapsed by default with a human-readable header line visible and the
technical detail hidden behind a toggle — settled at the module that owns the scanner's log UI, not
during the ownership refactor. §10 (D59 added), §11 (Q43 opened), §15 annotated.

The prior revision (v1.39 — 2026-09-02, owner-approved) records **D58 step 2
(Pane cluster) LANDED** in commit `0d4daf7` — a new pure class, `PaneState`, becomes the sole owner
of the pane decision's tab index and `readInProgress`; `showPane()` no longer reads
`tabLayout.selectedTabPosition` as an input at all, driving the `TabLayout`'s selection FROM
`PaneState` instead (behind a re-entry guard so a programmatic move is never misread as a user tap),
closing `.claude/remember/findings.md` #1 by construction — the framework's own, independently-timed
tab-position restore can no longer race the pane decision, because the pane decision no longer
consults it. `readInProgress` moves into the same owner (both its writers were already
read-lifecycle-only); `HandoffAdmission`'s `av://` guard is updated mechanically to the new location,
semantics unchanged. Unit tests 151→162 (JUnit XML, 0 failures; new `PaneStateTest`, 11 tests,
failing-first demonstrated as 30 compile errors). Device-confirmed on the Pixel 6a (pid 17066) across
four cases (Log-then-recreate, Scan-then-recreate, Log-reselect-then-recreate, and two back-to-back
recreations on Scan), tab state verified via a filtered `uiautomator dump` rather than a screenshot
(the scan form renders real MRZ fields). Separately, finding #4's open question was traced (not
fixed): the lock guard (`lockModeAndArm`) never reads `lastMrzHash` and behaves correctly after a
rotation; what remains open is `lastMrzHash`'s diagnostic-only mislabel and the finding's other lost
fields — finding #4 stays OPEN, narrowed. Finding #6 is unaffected — its `readInProgress` reference
moved location alongside this step, but the NFC-tag guard it names still never consults it. See
`docs/logs/M2-D58-STEP2-EVIDENCE.md`. §10 (D58's status line updated), §15 annotated.

The prior revision (v1.38 — 2026-09-02, owner-approved) records **D58 step 1
(Report/Log cluster) LANDED** in commit `c856f42` — `ReportLog` absorbs `lastReportText`;
`ReportLog.append`/`ReportLog.restore` are the cluster's only writers; `MainActivity.emitReport` and
its new named sibling `MainActivity.restoreReport` are thin renderers (closes
`.claude/remember/findings.md` #7 by construction). `ReportLog.entries` bounded at a named
`MAX_ENTRIES = 200` constant, oldest-first eviction (closes findings.md #13 FIXED-PROVISIONAL; the
number is PROPOSED pending owner approval, sized from rendered-fixture estimates, not
device-measured). Unit tests 145→151 (JUnit XML, 0 failures); device-confirmed on the Pixel 6a (pid
15939) across a forced config-change recreation, the restore path firing twice with all three prior
scan entries intact. Steps 2-4 of D58 remain not started. See `docs/logs/M2-D58-STEP1-EVIDENCE.md`.
§10 (D58's status line updated), §15 annotated.

The prior revision (v1.37 — 2026-09-02, owner-approved) records **D57**, a
...
| D57 | *(2026-09-02, owner decision — FREEZE, not a feature)* After the M2 scanner reached ~4,780 unreviewed LOC across seven isolated agent rounds, and the same-session D55 pane-visibility bug (a real user-facing bug that shipped and stranded a user, root-caused only by direct code inspection) demonstrated the cost of that backlog, the owner froze new §6.2 items and enhancements and adopted a rule set for every subsequent agent spawn: the entry gate is FIX vs ENHANCEMENT, never size (LOC is never a gate); every agent spawn carries history for the state it touches (prior reports, relevant git history, and any matching entries from `.claude/remember/findings.md`); one writer per piece of mutable state; every async writer is fenced; a finding an agent is not fixing goes into its report and is forwarded via a DURABLE FILE, never a code comment — the durable file is `.claude/remember/findings.md` (new this session, seeded from a read-only ownership audit, `docs/logs/M2-OWNERSHIP-AUDIT-2026-09-02.md`); read-only audit precedes refactor. **MUST NOT:** any new §6.2 item, enhancement, or UX change (including Q38/Q39/Q40 below) lands before the freeze's exit criterion is met. **Exit criterion, all three required:** (1) every mutable UI/session field named in the ownership audit has a named single writer; (2) every async writer (the five unfenced `Thread{}` sites the audit found, `.claude/remember/findings.md` #5) is fenced against Activity lifecycle; (3) `.claude/remember/findings.md` has no OPEN entry of consequence HIGH. **Recording this decision is not itself new scope** — NO-GO #10's scope gate (nothing enters a milestone unless first written into the PRD) is satisfied by this row and stays intact; a freeze forecloses scope, it does not add any. Applies to every module/agent working `apps/scanner`, not only the fields the audit named explicitly. **Status update (2026-09-02, FIX, uncommitted at time of writing — new `LifecycleFence.kt`, new `LifecycleFenceTest.kt`, modified `MainActivity.kt`): exit criterion (2) is MET.** A pure `LifecycleFence` (`alive`/`retire()`/`passes()`), held per-Activity-instance, retired in a new `onDestroy` (the module's first-ever `onDestroy`), fences all 11 relevant sites (10 `runOnUiThread` blocks plus `ReadTask.onPostExecute`, an `AsyncTask` the owner put in scope alongside the five originally-named `Thread{}` sites) — closes `.claude/remember/findings.md` #5. Fence semantics owner-decided: drops a stale main-thread landing, never cancels or aborts in-flight work. Every drop and the retirement itself are logged (12 static, zero-interpolation messages). Unit tests 180→184 (new `LifecycleFenceTest`, 0 failures). Device-proven on the Pixel 6a (real NL ID card) across six tests, including a per-instance-construction proof no unit test can give (two in-process recreations then a clean mint on the third instance with zero drops) and a dropped post-mint landing after a real completed `direct_post`/tier-B verdict — see new finding #16 and `docs/logs/M2-FENCE-EVIDENCE.md`. **The freeze does NOT lift.** Criterion (1) was already met by D58 step 4; criterion (3) is NOT met — `.claude/remember/findings.md` #10/#11 remain OPEN, mitigated not closed. **Correction (2026-09-02, commit `72e0b2c`, following a `/branch-review` finding over the 44-commit branch): the "11 sites" / "exit criterion (2) is MET" claim above was WRONG IN SCOPE.** It enumerated fence targets syntactically (every `runOnUiThread` call plus `ReadTask.onPostExecute`) rather than by the actual hazard predicate, and missed `BiometricPrompt.AuthenticationCallback` — a main-thread landing in exactly the same sense, verified reachable in `androidx.biometric` 1.1.0 bytecode because `MainActivity`'s `AppCompatActivity`/`FragmentActivity` constructor path never registers the library's own `ON_DESTROY` reset observer. `onAuthenticationError` called `emitReport` then `showBlockingOutcomeDialog` (`AlertDialog.Builder(this).show()`) against a possibly-destroyed Activity. Fixed in `72e0b2c`: the same guard added to `onAuthenticationError` and, for defence-in-depth, `onAuthenticationFailed`; **13 fenced sites now, not 11**; unit tests 184/0 unchanged (clean `--rerun-tasks` build); a sweep of every other non-`runOnUiThread` main-thread landing by the correct predicate found no further gaps. **Stated precisely so the record does not read as continuously true: criterion (2) was NOT actually met between `b8e0e05` and `72e0b2c`; it IS met now, at 13 sites, from `72e0b2c` onward.** The freeze still does NOT lift — criterion (3) remains unmet. No device evidence exists for the `BiometricPrompt` gap or its fix — code/bytecode-verified only. Full record in `.claude/remember/findings.md` #5's status-change note and `docs/logs/M2-FENCE-EVIDENCE.md`'s follow-up section. |
| D58 | *(2026-09-02, owner-approved)* **The ownership refactor (D57's exit criterion) executes in this order:** (1) **Report/Log cluster** — smallest, most closed, lowest consequence; proves the single-owner pattern cheaply; includes a named `restoreReport`/`restoreLog` sibling of `emitReport` closing the `:288` doc/code mismatch (`.claude/remember/findings.md` #7). (2) **Pane cluster** — fixes the confirmed rotation bug (`.claude/remember/findings.md` #1) by construction: tab index persisted, `TabLayout` selection DRIVEN FROM pane state, not read by it; confined to `onSaveInstanceState`/`onCreate` — MUST NOT touch `onNewIntent`, which is Q39's separate product decision. (3) **Lock-time snapshot of the verified request**, threaded as a parameter — closes the three-writer `pendingHandoff`/`verifiedRequest` race (`.claude/remember/findings.md` #2/#3) by construction; deletes the cross-thread reads at `continueAfterRead`/`mintAndMaybeHandoff`; after it lands the `HandoffAdmission` guard (`.claude/remember/findings.md` #10) becomes redundant and is REMOVED. (4) **Re-derive the Session boundary** on the corrected call/state graph before deciding whether Session/Handoff/Lock is one class or two. **Rationale recorded:** change ordering follows risk-of-the-change and dependency, not the audit's bug ranking; the `730ef09` guard closed remote induction of the race, so the snapshot (step 3) is defence-in-depth for an ownership defect rather than the only thing standing in front of an exploit. **CONDITION, verbatim: the snapshot MUST land — if the sequence stalls after step 2 the ownership bug is live and the record must say so.** **Pass-2 result, recorded as the basis for this ordering:** 6 clusters found — 3 in `MainActivity` (Session, Pane, Report/Log) and 3 already-correct (`ReportLog` internals, `DeviceKey.lastMintAlias`, `DeviceKey.softwareEd25519Store`); the owner's two-or-three-cluster hypothesis holds for the broken part and not as literally stated; "a class for errors" has no corresponding mutable state (`FailureTransition` is a stateless classifier already at ceiling). The structure proposal itself lives in the session scratchpad — it is NOT copied into this document until step 4 re-derives it. **Status update (2026-09-02, commit `c856f42`): step 1 (Report/Log cluster) LANDED** — `ReportLog` absorbs `lastReportText`; `ReportLog.append`/`ReportLog.restore` are the cluster's only writers; `MainActivity.emitReport` and its new named sibling `MainActivity.restoreReport` are thin renderers (closes `.claude/remember/findings.md` #7 by construction). `ReportLog.entries` bounded at a named `MAX_ENTRIES = 200` constant, oldest-first eviction (closes findings.md #13 FIXED-PROVISIONAL; the number is PROPOSED pending owner approval, sized from rendered-fixture estimates, not device-measured). Unit tests 145→151 (JUnit XML, 0 failures); device-confirmed on the Pixel 6a (pid 15939) across a forced config-change recreation, restore firing twice with all three prior scan entries intact. Steps 2-4 remain not started. See `docs/logs/M2-D58-STEP1-EVIDENCE.md`. **Status update (2026-09-02, commit `0d4daf7`): step 2 (Pane cluster) LANDED** — a new pure class, `PaneState`, becomes the sole owner of the pane decision's tab index and `readInProgress`; `showPane()` no longer reads `tabLayout.selectedTabPosition` as an input, driving the `TabLayout` selection FROM `PaneState` instead (re-entry guard against misreading a programmatic move as a user tap); closes `.claude/remember/findings.md` #1 by construction — the framework's own tab-position restore can no longer race the pane decision, because the pane decision no longer consults it. `readInProgress` moves into the same owner (both its writers were already read-lifecycle-only); `HandoffAdmission`'s `av://` guard updated mechanically to the new location, semantics unchanged. Unit tests 151→162 (JUnit XML, 0 failures; new `PaneStateTest`, 11 tests; failing-first demonstrated as 30 compile errors). Device-confirmed on the Pixel 6a (pid 17066) across four cases (Log/Scan/Log-reselect/back-to-back recreations, the last added by the orchestrator), tab state verified via a filtered `uiautomator dump` rather than a screenshot (the scan form renders real MRZ fields, unsafe to snapshot). Finding #4's open question was traced (not fixed) as a report-only deliverable: the lock guard never reads `lastMrzHash` and behaves correctly after a rotation; what remains open is `lastMrzHash`'s diagnostic-only mislabel and the finding's other lost fields — #4 stays OPEN, narrowed, not closed. Finding #6 is unaffected — its `readInProgress` reference moved location but the NFC-tag guard it names still never consults it. Step 3 (lock-time snapshot) and step 4 (re-derived Session boundary) remain not started. See `docs/logs/M2-D58-STEP2-EVIDENCE.md`. **Status update (2026-09-02, commit `65096b9`): step 3 (lock-time snapshot) LANDED — D58's verbatim condition is met.** A new immutable `data class AuthorizedHandoff(request, origin, site)` is captured exactly once, on the main thread, in `lockModeAndArm`, and threaded as a parameter through the whole read/mint pipeline (`startSession` → `ReadTask` → `continueAfterRead` → `promptAndMint` → `mintAndMaybeHandoff`); both cross-thread reads of `pendingHandoff`/`verifiedRequest` are deleted, closing `.claude/remember/findings.md` #2/#3 by construction with no staleness guard needed. **`HandoffAdmission` was KEPT, not removed, contrary to this row's own stated expectation.** Reason, verified at source: `applyHandoffVerificationOutcome` unconditionally calls `refreshModeStatus()` and re-enables `lockButton` on a successful verify without ever checking `lockedMode`, and only `HandoffAdmission` refusing the intent before that callback runs prevents an admitted foreign intent from stomping the "Locked: mode X" banner (inert for re-locking, but a real display regression) — mint correctness itself does not depend on the guard, only that separate, still-live projection defect does. **Guard removal now unblocks only once `applyHandoffVerificationOutcome` itself respects `lockedMode`** — that is step 4's job (`modeStatusView`/`lockButton.isEnabled` are on this step's own MUST NOT list), not a new step. Unit tests 162→168 (JUnit XML, 0 failures; new `AuthorizedHandoffTest`, including a supersession regression test and a reflection check that every field stays `final`); the one remaining `!!` (at the NFC-tag call site) was verified, not assumed, safe via two independent gates in sequence (`MintGate`'s mode-A-never-mints test; the pre-existing D38 origin guard for mode B with a null snapshot). Device-confirmed on the Pixel 6a across two sessions, real NL ID card: `HandoffAdmission` correctly refused four hostile `av://` transactions across three timing windows (post-lock/pre-read, mid-read via an automated logcat-triggered watcher landing 60ms after the card tap, and one missed-window case correctly accepted once the session was already consumed), one refusal independently confirmed user-visible (Snackbar seen mid-scan); legitimate mints in both sessions completed against the transaction actually locked, cross-checked against the verifier's own independent pending/done state rather than the device's self-report alone — a cross-check that caught the orchestrator's own initial mistracking of which transaction had completed. **Stated limitations**: all hostile links used the same local verifier origin (a genuinely foreign origin is untested); the PIN-prompt window was not isolated; the QR-scan/manual-paste handoff path (`applyPendingHandoffText`) has NO admission gate at all — a pre-existing, same-class gap found by this step's own required survey, recorded as `.claude/remember/findings.md` #15. Full evidence in `docs/logs/M2-D58-STEP3-EVIDENCE.md`. **Status update (2026-09-02, commit `c38833d`): step 4 (re-derived Session boundary) LANDED IN PART — all four D58 steps have now landed.** A new pure class `SessionDisplay` (`render(lockedMode, handoffState)`, `HandoffState` sealed as `None`/`Verifying`/`Verified(origin, tier)`/`Refused(reason)`) becomes the SOLE deriver of the three write-only projections this step's own naming targeted; `MainActivity.applySessionDisplay` is verified the sole writer of `modeStatusView.text`, `handoffStatus.text`, `lockButton.isEnabled`, and `lockButton.text` (only `MainActivity.kt:430-433` assign them), collapsing six prior write sites into one and deleting `refreshModeStatus`. Locked state takes UNCONDITIONAL precedence in `render`, so an admitted foreign verification cannot alter a locked session's display even with `HandoffAdmission` removed — closing `.claude/remember/findings.md` #14 (stale handoff status survives a consumed session) and #9 (`lockButton.isEnabled` four-writer join) by construction, and closing #15 (the QR-scan/manual-paste path had no `HandoffAdmission` gate at all) by applying the same admission predicate and refusal shape (`Log.e` + Snackbar + return, no log entry per #13, no `showBlockingOutcomeDialog` per #12) to `applyPendingHandoffText` as the `av://` path already had. **`HandoffAdmission` was KEPT and no `SessionState` class was extracted — both were recommendations only, not this step's requirement.** The reason step 3 gave for keeping the guard (`applyHandoffVerificationOutcome` rewriting the display without respecting `lockedMode`) is now closed by `SessionDisplay`'s locked-wins precedence; what the guard still buys, narrowed precisely, is preventing a foreign handoff from overwriting the mutable `pendingHandoff`/`verifiedRequest` fields while locked or reading — field overwrite only, no longer display corruption or mint correctness, both closed elsewhere. **Behaviour change, flagged not hidden**: `wipeSession` no longer force-enables the Lock button, so it stays disabled during an in-flight verification rather than contradicting the "verifying" banner; verified this cannot strand a user via the two network call sites' 10-second timeouts (`HandoffClient.kt:87-88,156-157`, `RequestTrust.kt:150-151`) and the background worker posting an outcome even on exception, so every terminating path re-renders. **Job 4's own deliverable — the re-derived Session boundary — was reported, deliberately NOT implemented**: the coder recommends TWO units, not the one `SessionState` the structure proposal proposed — (a) a session-state holder for `lockedMode`/`authorizedHandoff`/`pendingHandoff`/`verifiedRequest`/`lastMrzHash`, and (b) `SessionDisplay` as a separate stateless projection, because folding a stateless decision into a stateful holder loses the testability the project's `PaneVisibility`/`PaneState` precedent depends on. The display boundary has SIX legal projections (locked-A and locked-B each collapse to one projection regardless of handoff state, plus unlocked crossed with the four handoff states); the underlying mutable fields have more reachable combinations than that, but they are invisible rather than illegal, which is exactly what the locked-wins rule buys. **The structure proposal's cluster 1 is now STALE on two points**: its proposed `@Volatile`/CAS synchronization was made moot by step 3's parameter threading, and its proposal to fold the three views in as owned fields is superseded by modelling them as a projection. Unit tests 168→180 (JUnit XML, 0 failures; new `SessionDisplayTest`, 12 tests). Device-confirmed on the Pixel 6a (pid 21642): two mode-B mints completed end-to-end, and a subsequent scan on the already-consumed session correctly ran as a local read with no mint-gate evidence — finding #14's exact scenario, confirmed not to recur at the data/log level. **Caveat, stated plainly: the on-screen `handoffStatus.text` line itself was NOT visually re-verified this session** (Log-tab visibility and a notification-shade interruption both prevented it) — confirmed at the log/behaviour level only. Owner UX gap noted, not a defect of this fix: no popup/Snackbar distinguishes a consumed-session local read from any other scan, and the status line lives only on the Scan tab. With #9/#13/#14/#15 closed and #10/#11 mitigated, finding #5 (zero async-cancellation discipline) is now the PRINCIPAL REMAINING BLOCKER on D57's exit criterion. Full evidence in `docs/logs/M2-D58-STEP4-EVIDENCE.md`. |
| D59 | *(2026-09-02, owner decision, amends D58 step 1's `MAX_ENTRIES` status — original D58 text kept, not deleted)* **The `ReportLog` bound is 20 entries, not 200.** D58 step 1 (commit `c856f42`) introduced `ReportLog.MAX_ENTRIES = 200` as an explicitly PROVISIONAL number pending owner approval, closing `.claude/remember/findings.md` #13 (unbounded, externally-triggerable persisted growth) FIXED-PROVISIONAL. The owner has now clarified the unit and set the number: the cap counts ENTRIES, where one entry is one whole scan-outcome block of roughly 20 rendered lines, NOT lines — 200 entries would be an unusable scroll. **Approved cap: 20 entries, oldest-first eviction, mechanism unchanged.** **Status update (2026-09-02, commit `ff15629`): LANDED.** `ReportLog.MAX_ENTRIES` is now `20`, replacing the provisional `200` from `c856f42`; both bound tests already referenced the constant symbolically rather than by literal, so they scaled untouched — tests 168/0/0, unchanged. Closes `.claude/remember/findings.md` #13 as **FIXED** (no longer provisional). |
| D60 | *(2026-09-02, owner decision — branch close-out; the freeze from D57 is CARRIED FORWARD, NOT lifted)* **Owner's decision, in their own words: "we will exit here to close this branch... we can move to release and future freeze still there, we will take it with next module first thing, a continuation" and "i want to pause this and move on, otherwise its a perpetual delay."** **Stated reason, recorded as the justification for closing with a freeze still in force**: an open-ended chain of fix-then-re-review (the `b8e0e05` → `72e0b2c` → `9584bc8` sequence this branch just ran) would otherwise delay the branch indefinitely; pausing here stops that chain rather than letting it continue unbounded. **D57's exit criteria, status at close**: (1) MET — every mutable UI/session field named in the ownership audit has a named single writer (D58 steps 1-4). (2) MET as of `72e0b2c` — every async landing is fenced against Activity lifecycle, 13 sites (11 from `b8e0e05` plus 2 `BiometricPrompt` callback sites added by `72e0b2c`'s correction — see D57's own status line and PRD v1.44). (3) **NOT MET** — `.claude/remember/findings.md` #10 and #11 remain OPEN at consequence HIGH, mitigated not closed; these two alone are why the freeze does not lift at close. **This is a DEFERRAL, not a lift: no new §6.2 item, enhancement, or UX change may land while D57 stands, and every UI item already deferred under D57 (Q43-Q48) stays deferred.** Clearing D57's exit criterion (3) is now the FIRST work item of the next module, before that module's own work — not folded into it, not optional, not re-opened for debate at that module's start. **Recording this decision is not itself new scope** — NO-GO #10's scope gate is satisfied by this row and stays intact; a deferral forecloses scope, it does not add any. **PENDING-AT-CLOSE, verified against `.claude/remember/findings.md` before writing (not copied on trust), so the next module inherits a complete picture rather than a summary:** **Blocking the freeze's exit** — findings **#10** (`av://` intent hijack mid-session) and **#11** (biometric prompt shows no origin/site/tier) — both OPEN, consequence HIGH, MITIGATED (commit `730ef09`) not closed; guard (`HandoffAdmission`) kept by recommendation only, its removal blocked on resolving these two. **Open, not freeze-blocking** — **#4** (11 of 16 fields lost on rotation; narrowed but not closed by D58 step 2's trace), **#6** (`handleIncomingIntent` tag guard ignores `readInProgress`), **#8** (chip-auth three-state logic has no unit test, DG14 handling inline), **#16** (mint-report loss after Activity destruction — OPEN **by owner decision**, a disclosure/product question, not an oversight left undone). **Answered but not decided** — **Q38** (log lifetime): device-confirmed `ReportLog` survives Activity recreation via saved instance state, nothing survives process death; the owner has not decided whether a completed presentation needs a durable, destruction-surviving on-device record. **Deferred UI items, all still deferred** — **Q43** (collapsible log entries), **Q44** (dim a completed run with ticked checkboxes), **Q45** (a single "verify" vs. "scan local" control), **Q46** ("Passport number" label correctness), **Q47** (input-focus steal corrupting MRZ entry), **Q48** (three indistinguishable reader apps on the launcher). **Verification debt** — the `72e0b2c` `BiometricPrompt` fence guard has code/bytecode evidence only, no device run; settling it needs a device test that destroys the Activity while a biometric prompt is outstanding, confirming the landing drops with no `WindowManager$BadTokenException`. Separately, the QR-scan/manual-paste handoff path (`applyPendingHandoffText`) has never been exercised under fence conditions on device — only the `av://` path has device evidence for fence behaviour. **One open fix-ledger bullet** (`.claude/remember/fix-ledger.md`, added by the re-review at `9584bc8`, not this document's own artifact): `LifecycleFence.kt`'s class-doc thread-safety proof enumerates only two safe-read forms (`runOnUiThread`, `onPostExecute`) and is now stale — `72e0b2c` added a third (a main-executor-dispatched `BiometricPrompt.AuthenticationCallback`). Doc-drift only, no behavioural defect; consumed by `/refactor`, not tracked further here. |
...
- **Q38 (opened 2026-09-02, deferred under D57) — does the in-app log need to survive app close?**
  ...
  single-writer state the freeze is protecting from a fifth uncoordinated change. **Status update
  (2026-09-02) — device evidence, not a decision:** `ReportLog` survives Activity recreation via
  saved instance state (confirmed repeatedly across this session's device runs); NOTHING survives
  process death — there is no disk persistence at all, evidenced by the owner's own observation
  ("logs don't survive after I close the app") plus three process-id changes across the session
  (23196 → 24123 → 24361 → 24553). This narrows the question but does not resolve it: whether a
  completed presentation needs a durable, destruction-surviving on-device record is now also the
  substance of new finding #16 (`.claude/remember/findings.md`) — the same question from a different
  angle. No approach proposed or chosen; still blocked by D57. See `docs/logs/M2-FENCE-EVIDENCE.md`.
...
- **Q40 (opened 2026-09-02, deferred under D57) — "stuck"-reading Lock button after an
  ...
  would otherwise touch without resolving. **Status update (2026-09-02, commit `c38833d`):** the
  structure pass this note required has now landed (D58 step 4, `SessionDisplay`) and
  `lockButton.isEnabled`/`.text` have a single named writer (`.claude/remember/findings.md` #9,
  CLOSED). The Lock button's label reads **"Tap and scan"** when locked, shipped in this same commit —
  recorded as **PROVISIONAL pending owner approval**, since this question's own wording/mechanism was
  never formally decided before the structure pass landed; the ownership precondition this note named
  is satisfied, but the copy itself still awaits an explicit owner sign-off.
...
- **Q43 (opened 2026-09-02, deferred under D57 — UI/UX enhancement, not a fix)** — collapsible log
  entries. Owner observation: each `ReportLog` entry is roughly 20 lines, so a plain list of them is
  a massive scroll; entries should be COLLAPSED by default, showing a human-readable header line,
  with the technical detail hidden behind a toggle. This is a UI/UX ENHANCEMENT under D57's freeze —
  MUST NOT be built now — and, per NO-GO #10's scope gate, cannot enter a milestone without first
  being written into this document, which this entry does. No design proposed here; settled at the
  module that owns the scanner's log UI, not during the ownership refactor.

- **Q44 (opened 2026-09-02, deferred under D57 — UI/UX enhancement, not a fix)** — dim a completed
  run, with ticked checkboxes. Owner observation from the D58 step 4 device run: nothing tells the
  user a run is DONE. The owner's proposed shape: a successful run's display should DIM once complete,
  with its checks shown as ticked boxes, so a finished scan is visually distinct at a glance. This is a
  UI/UX ENHANCEMENT under D57's freeze — MUST NOT be built now — and, per NO-GO #10's scope gate,
  cannot enter a milestone without first being written into this document, which this entry does. No
  design proposed beyond the owner's stated shape; settled at the UI module alongside Q43.

- **Q45 (opened 2026-09-02, deferred under D57 — UI/UX enhancement, not a fix)** — a single control
  distinguishing "verify" from "scan local." Owner observation: a reset button is not needed, but a
  single control labelled to distinguish the two kinds of scan a user might perform — along the lines
  of "verify" versus "scan local" — would let the user know in advance which kind of scan they are
  about to run. Recorded as an OPEN SHAPE, not a decided design — no wording or mechanism proposed
  beyond the owner's own framing. UI/UX ENHANCEMENT under D57's freeze — MUST NOT be built now; settled
  at the UI module alongside Q43/Q44.

- **Q46 (opened 2026-09-02, deferred under D57 — label CORRECTNESS defect, not a styling preference)**
  — the MRZ input field is labelled "Passport number." That is factually wrong for this app: it reads
  national ID cards as well as passports, and was validated on an NL ID card (M0/M2 evidence). It
  should read "Document number" or "Passport/ID." Recorded explicitly as a correctness defect, not a
  styling enhancement — but still deferred to the same UI pass as Q43/Q44/Q45 under D57's freeze,
  rather than actioned standalone, since it touches the same scan-form surface the freeze is
  protecting from uncoordinated changes.

- **Q47 (opened 2026-09-02, deferred under D57 — input-focus CORRECTNESS defect, not a styling
  preference)** — input focus steals back to the document-number field. Device evidence from the
  2026-09-02 fence run: while typing the date fields on the scan form, the cursor jumps back to the
  document-number field, corrupting the MRZ already entered. This has a MEASURED COST, not just an
  annoyance: it is the suspected (not confirmed) cause of a `SW 0x6985`
  (`org.jmrtd.AccessDeniedException`, D56's known access-establishment signature for wrong MRZ
  input) read failure that destroyed one T5 fence-test run during that same session — see
  `docs/logs/M2-FENCE-EVIDENCE.md`. No root-cause isolation was performed; the causal link is
  suspected from timing adjacency only. Sits on the same scan-form screen as Q46's "Passport number"
  label defect. Deferred to the same UI pass as Q43/Q44/Q45/Q46 under D57's freeze.

- **Q48 (opened 2026-09-02, deferred under D57 — UI/UX enhancement, human-identification only, NOT
  a security question)** — the three installed reader apps are indistinguishable on the launcher.
  Device evidence: `com.zkagent.scanner` (0.1.0), `com.tananaev.passportreader` (3.4, the M0 fork),
  and `com.zkagent.m2sessionpoc` (0.1-spike) are all installed on the same dev device with generic
  launcher icons/labels that do not distinguish them. Verified fact, not assumption: `cmd package
  query-activities -a android.intent.action.VIEW -d 'av://authorize'` resolves to
  `com.zkagent.scanner` ALONE, so `av://` intent routing on this device is deterministic — this
  finding is a human mis-identification risk (the owner picking the wrong app), not a security gap.
  **Explicitly does NOT bear on findings #10/#11** (`.claude/remember/findings.md`): a hostile
  third-party app registering its own `av://` filter would still produce a disambiguation chooser
  regardless of how many of zkagent's own dev builds are installed, and that intent-hijack finding's
  mitigation state is unchanged by this observation. Suggested shape, owner's, not decided: distinct
  launcher labels carrying the version string. UI/UX ENHANCEMENT under D57's freeze — MUST NOT be
  built now; settled at the UI module alongside Q43/Q44/Q45/Q46/Q47.
...
| Version | Date | Change |
|---|---|---|
| v1.46 — 2026-09-02, pending owner approval | 2026-09-02 | DOCS reconciliation on `chore/memory-consolidation`, no code change. `.claude/remember/findings.md`: #6 and #8 marked FIXED, each with residuals stated; #5's `fix-ledger.md` bullet cleared; #10/#11 given RECONCILIATION notes on what remains to close, pending owner ruling; #4 unaffected. Freeze D57/D60 unchanged. No device attached — verification debt unchanged. New Snackbar wording pending owner approval. Detail: `.claude/remember/findings.md`, `CHANGELOG.md` `[Unreleased]`. §15 only. |
| v1.45 — 2026-09-02, owner-approved | 2026-09-02 | **D60 added: branch close-out — the D57 freeze is CARRIED FORWARD, not lifted and not abandoned.** Owner's own words recorded verbatim: "we will exit here to close this branch... we can move to release and future freeze still there, we will take it with next module first thing, a continuation" / "i want to pause this and move on, otherwise its a perpetual delay." Stated reason: stopping an open-ended fix-then-re-review chain (`b8e0e05` → `72e0b2c` → `9584bc8`) from delaying the branch indefinitely. **Exit-criteria status at close**: (1) MET, (2) MET as of `72e0b2c` (13 fenced sites), (3) NOT MET — findings #10/#11 remain OPEN at consequence HIGH, mitigated not closed; these two are the entire reason the freeze does not lift. **This is a deferral, not a lift**: no new §6.2 item/enhancement/UX change lands while D57 stands; Q43-Q48 stay deferred. Clearing criterion (3) is the FIRST work item of the next module, ahead of that module's own work. **A verified-against-source PENDING-AT-CLOSE list is recorded in D60 itself**: blocking (#10/#11), open-not-blocking (#4, #6, #8, #16), answered-not-decided (Q38), deferred UI (Q43-Q48), verification debt (the `72e0b2c` `BiometricPrompt` fence has no device evidence; the QR-scan/manual-paste path has never been exercised under fence conditions on device), and one open `.claude/remember/fix-ledger.md` bullet (`LifecycleFence.kt`'s class doc is stale after `72e0b2c`, doc-drift only, consumed by `/refactor`). No new questions opened, no finding or question closed, no renumbering, D57's own row NOT annotated as lifted. §10 (D60 added), §15 annotated. |
| v1.44 — 2026-09-02, owner-approved | 2026-09-02 | **Correction to v1.43's finding #5 record, following a `/branch-review` pass over the 44-commit branch that found a real gap in the fix v1.43 recorded (v1.43's records were committed as `2a4db81`; the corrective fix landed as `72e0b2c`).** The "11 sites fenced" / "exit criterion (2) is MET" claim v1.43 recorded was WRONG IN SCOPE: it enumerated fence targets syntactically (every `runOnUiThread` call plus `ReadTask.onPostExecute`), not by the actual hazard predicate, and missed `BiometricPrompt.AuthenticationCallback` — a main-thread landing dispatched on the main executor, in exactly the same sense as the 11 sites already fenced. `onAuthenticationError` called `emitReport` then `showBlockingOutcomeDialog` (`AlertDialog.Builder(this).show()`) against a possibly-destroyed Activity. **Reachability bytecode-verified by the orchestrator in `androidx.biometric` 1.1.0, not assumed**: `BiometricPrompt$ResetCallbackObserver.resetCallback()` is `@OnLifecycleEvent(ON_DESTROY)` and nulls `BiometricViewModel.mClientCallback` (after which `getClientCallback()` substitutes a no-op default) — but `addObservers()` is invoked ONLY from the library's two `Fragment` constructors, and `MainActivity`, an `AppCompatActivity` (`:186`) using the `FragmentActivity` overload (`:1779`), never calls it — so that protection is not active here, one constructor overload away from being true. **Fix (`72e0b2c`)**: the same guard added to `onAuthenticationError`, plus `onAuthenticationFailed` for defence-in-depth (placed after its existing diagnostic log line so it never suppresses it); `onAuthenticationSucceeded` unchanged (only starts an already-fenced `Thread{}`); both new messages static, zero interpolation, `Log.i` not `Log.w`. **13 fenced sites now, not 11.** Unit tests 184/0 unchanged, verified on a clean `--rerun-tasks` build, exit 0. **Sweep** of every other non-`runOnUiThread` main-thread landing in `MainActivity.kt`, by the corrected predicate ("a callback the framework may deliver late that touches Activity-owned UI or state," not text match), found no further gaps: click/editor/long-click listeners (alive by construction), `registerForActivityResult`/`qrCaptureLauncher` (lifecycle-aware `ActivityResultRegistry`), the `DatePickerDialog` `DialogFragment` (torn down with its host), NFC `onNewIntent` dispatch (synchronous, resumed-only), `DeviceKey.exportDevAttesterPublicKeyIfPresent` (no Activity landing exists to fence). **No device evidence exists for this gap or its fix** — the six device tests in `docs/logs/M2-FENCE-EVIDENCE.md` (T1-T5, T2 included) did not exercise the `BiometricPrompt` callback path; settling it needs a device test that destroys the Activity while a prompt is outstanding, confirming the landing drops with no `WindowManager$BadTokenException` — not yet run. **Criterion (2) stated precisely, so the record does not read as continuously true: it was NOT actually met between `b8e0e05` and `72e0b2c`; it IS met now, at 13 sites, only from `72e0b2c` onward.** **The freeze does NOT lift** — criterion (3) remains unmet, `.claude/remember/findings.md` #10/#11 remain OPEN, mitigated not closed. **Recurrence-prevention lesson, recorded in the project's own voice**: enumerate async landings by the predicate "a callback the framework may deliver late that touches Activity-owned UI or state," never by grepping for a syntactic form such as `runOnUiThread`. No new questions opened, no decisions invented, no renumbering. §10 (D57's status line annotated in place with this correction), `.claude/remember/findings.md` (#5 status-change note appended, no new entry), `docs/logs/M2-FENCE-EVIDENCE.md` (follow-up section appended), §15 annotated. |
| v1.43 — 2026-09-02, owner-approved | 2026-09-02 | Findings.md #5 (zero async-cancellation discipline) FIXED, device-proven — D57's exit criterion (2) is MET. An UNCOMMITTED FIX pass (new `LifecycleFence.kt`, new `LifecycleFenceTest.kt`, modified `MainActivity.kt`) adds a pure per-Activity-instance `LifecycleFence` (`alive`/`retire()`/`passes()`), retired in a new `onDestroy` (the module's first-ever `onDestroy`), fencing 11 sites (10 `runOnUiThread` blocks plus `ReadTask.onPostExecute`, an `AsyncTask` the owner put in scope alongside the five originally-named `Thread{}` sites — same defect class). Owner-decided semantics: a fence drops a stale main-thread landing, it never cancels or aborts in-flight work. Every drop and the retirement are logged (12 static, zero-interpolation messages; one `Log.w`, the rest `Log.i`). Unit tests 180→184 (new `LifecycleFenceTest`, 0 failures). **Device-proven on the Pixel 6a (real NL ID card, verifier spike `spikes/m2-handoff`) across six tests**: T1 three verifier-cross-checked happy-path mints; T2 proved per-instance construction — a property no unit test can give — via two in-process Activity recreations (font-scale toggle, not a process restart) followed by a clean mint on the third instance with zero fence-drop lines; T3 dropped a mid-verification landing cleanly; T4 dropped a mid-read landing with no mint and no stranded UI (D55 invariant held, confirmed via a filtered accessibility read, never a screenshot); T5 dropped a post-mint landing AFTER a real `direct_post` 200 and a verifier tier-B evidence-verified verdict — evidence left the device with nothing recorded in `ReportLog` and nothing shown to the user beyond the one `Log.w` line, opening **new finding #16**. T3/T5's drop windows were only reachable via an artificial test-harness delay proxy (unmodified app and verifier; natural windows ~12-28ms on localhost); the QR-scan/manual-paste path was not exercised under fence conditions. **The freeze does NOT lift** — exit criterion (3) is not met, `.claude/remember/findings.md` #10/#11 remain OPEN, mitigated not closed. **Q38 (log lifetime) answered by device evidence** (not decided): `ReportLog` survives Activity recreation via saved instance state; nothing survives process death — confirmed via three process-id changes across the session and the owner's own observation. **Two new open questions**: **Q47** — input focus steals back to the document-number field while typing date fields, corrupting entered MRZ; a CORRECTNESS defect, suspected (not confirmed) cause of one T5 run's `SW 0x6985` access-establishment failure; deferred under D57 alongside Q43-Q46. **Q48** — the three installed reader apps (`com.zkagent.scanner`, `com.tananaev.passportreader`, `com.zkagent.m2sessionpoc`) are indistinguishable on the launcher; verified only `com.zkagent.scanner` declares the `av://` VIEW filter (`cmd package query-activities`), so intent routing is deterministic and this is a human-identification problem, not a security one — does NOT bear on findings #10/#11. Full evidence in `docs/logs/M2-FENCE-EVIDENCE.md`. §10 (D57's exit-criterion status annotated in place, no renumbering), §11 (Q38 status update, Q47/Q48 opened), `.claude/remember/findings.md` (#5 status update, new #16), §15 annotated. |
| v1.42 — 2026-09-02, owner-approved | 2026-09-02 | D58 step 4 (last of D58's four steps — all four now LANDED) — the re-derived Session boundary, landed in commit `c38833d` as `SessionDisplay` projections plus an admission-gated paste path: a new pure `SessionDisplay.render(lockedMode, handoffState)` (`HandoffState` sealed as `None`/`Verifying`/`Verified(origin,tier)`/`Refused(reason)`) becomes the sole deriver of `modeStatusView.text`/`handoffStatus.text`/`lockButton.isEnabled`/`.text`, verified as the sole writer (`MainActivity.kt:430-433`), collapsing six prior write sites and deleting `refreshModeStatus`. Locked state takes unconditional precedence, closing `.claude/remember/findings.md` #14 (stale handoff status after a consumed session) and #9 (`lockButton.isEnabled` four-writer join) by construction, and #15 (QR-scan/manual-paste path had no admission gate) by applying `HandoffAdmission`'s predicate and refusal shape (`Log.e` + Snackbar + return, no log entry per #13, no `showBlockingOutcomeDialog` per #12) to `applyPendingHandoffText` as well. **`HandoffAdmission` was KEPT and no `SessionState` class extracted — both recommendations only.** The reason step 3 gave for keeping the guard is now closed by `SessionDisplay`'s locked-wins precedence; its remaining value is narrowed to preventing field overwrite of `pendingHandoff`/`verifiedRequest` while locked or reading — mint correctness and display corruption no longer depend on it (findings #10/#11 updated in place: still MITIGATED, guard kept by recommendation, not a code dependency). Behaviour change flagged not hidden: `wipeSession` no longer force-enables the Lock button; verified this cannot strand a user via the two network call sites' 10-second timeouts and the background worker's guaranteed outcome-posting on every terminating path. **Job 4's own deliverable — the re-derived Session boundary — was reported, deliberately NOT implemented**: the coder recommends two units (a session-state holder plus `SessionDisplay` as a separate stateless projection), not the single `SessionState` the structure proposal proposed; the display boundary has six legal projections (locked-A/locked-B each collapse to one regardless of handoff state, plus unlocked crossed with the four handoff states); the structure proposal's cluster 1 is now STALE on two points (its `@Volatile`/CAS proposal mooted by step 3, its owned-fields proposal superseded by the projection model). Unit tests 168→180 (JUnit XML, 0 failures; new `SessionDisplayTest`, 12 tests, including one named for finding #14's exact defect and two locked-wins tests). Device-confirmed on the Pixel 6a (pid 21642): two mode-B mints completed end-to-end, and a scan on the already-consumed session correctly ran as a local read with no mint-gate evidence — finding #14's exact scenario, confirmed not to recur at the data/log level — but the on-screen `handoffStatus.text` line itself was NOT visually re-verified this session (Log-tab visibility and a notification-shade interruption both prevented it). Owner UX gap noted, not a defect of this fix: no popup/Snackbar distinguishes a consumed-session local read from any other scan, and the status line lives only on the Scan tab. Three new owner UI items opened as **Q44/Q45/Q46**: dimming a completed run with ticked checkboxes, a single "verify" vs. "scan local" control, and a label-correctness defect ("Passport number" should read "Document number"/"Passport/ID") — all deferred under D57 alongside Q43. **Q40's "Tap and scan" Lock label shipped in `c38833d`, noted PROVISIONAL pending owner approval.** With #9/#13/#14/#15 closed and #10/#11 mitigated, finding #5 (zero async-cancellation discipline) is now the PRINCIPAL REMAINING BLOCKER on D57's exit criterion. Full evidence in `docs/logs/M2-D58-STEP4-EVIDENCE.md`. §10 (D58's status line updated in place, no renumbering, no new decisions), §11 (Q40 annotated, Q44/Q45/Q46 opened), §15 are annotated. |
| v1.41 — 2026-09-02, owner-approved | 2026-09-02 | D58 step 3 (amends item in D58's execution order, original D58 text kept) — the lock-time verified-request snapshot, third of D58's four steps, LANDED in commit `65096b9`: a new immutable `data class AuthorizedHandoff(request, origin, site)` is captured exactly once, on the main thread, in `lockModeAndArm`, and threaded as a parameter through `startSession` → `ReadTask` → `continueAfterRead` → `promptAndMint` → `mintAndMaybeHandoff`; both cross-thread reads of `pendingHandoff`/`verifiedRequest` are deleted, closing `.claude/remember/findings.md` #2/#3 by construction with no staleness guard or generation counter needed — a later `beginHandoffVerification` call cannot alter an already-captured snapshot (`val`, no setter; a reflection test in the new `AuthorizedHandoffTest` fails if any field ever becomes `var`). The one remaining `!!` (at the NFC-tag call site) was verified, not assumed, safe: mode A returns early at the `MintGate` check (a test literally named "mode A NEVER mints, even with ok true allowed true"), and mode B with a null snapshot is refused earlier by the pre-existing D38 origin guard — two gates in sequence. **`HandoffAdmission` was KEPT, not removed, contrary to this row's own stated expectation.** Reason, verified at source: `applyHandoffVerificationOutcome` unconditionally calls `refreshModeStatus()` and re-enables `lockButton` on a successful verify without ever checking `lockedMode`; only `HandoffAdmission` refusing the intent first prevents an admitted foreign intent from stomping the "Locked: mode X" banner (inert for re-locking, but a real display regression) — mint correctness does not depend on the guard, only that separate, still-live projection defect does. Guard removal now unblocks only once `applyHandoffVerificationOutcome` itself respects `lockedMode` — D58 step 4's job, since `modeStatusView`/`lockButton.isEnabled` are on this step's own MUST NOT list. Unit tests 162→168 (JUnit XML, 0 failures; new `AuthorizedHandoffTest`, supersession regression test plus the reflection check). Device-confirmed on the Pixel 6a across two sessions, real NL ID card: `HandoffAdmission` correctly refused four hostile `av://` transactions across three timing windows (post-lock/pre-read, mid-read via an automated logcat-triggered watcher landing 60ms after the card tap, and one missed-window case correctly accepted once the session was already consumed), one refusal independently confirmed user-visible (Snackbar seen mid-scan); legitimate mints in both sessions completed against the transaction actually locked, cross-checked against the verifier's own independent pending/done state rather than the device's self-report alone — catching the orchestrator's own initial mistracking of which transaction had completed, recorded as a lesson: a device log reporting success is not sufficient evidence of which transaction completed without an independent counterparty check. Stated limitations: all hostile links used the same local verifier origin (a genuinely foreign origin is untested); the PIN-prompt window was not isolated; the QR-scan/manual-paste handoff path (`applyPendingHandoffText`) has NO admission gate at all — a pre-existing, same-class gap found by this step's own required survey, recorded as `.claude/remember/findings.md` #15 (findings.md #10/#11 updated in place: still MITIGATED, not closed, guard removal blocked on step 4 as above). Findings.md #13 separately closes as FIXED (no longer provisional): D59's owner-approved `ReportLog.MAX_ENTRIES = 20` landed in commit `ff15629`, replacing the provisional `200` from D58 step 1; both bound tests already referenced the constant symbolically, so they scaled untouched (tests 168/0/0, unchanged). Step 4 (re-derived Session boundary) remains not started. Full evidence in `docs/logs/M2-D58-STEP3-EVIDENCE.md`. §10 (D58's status line and D59's status line both updated in place, no renumbering, no new decisions), §15 are annotated. |
| v1.40 — 2026-09-02, owner-approved | 2026-09-02 | Two owner decisions. **D59** clarifies D58 step 1's provisional `ReportLog.MAX_ENTRIES` constant (commit `c856f42`): the cap counts ENTRIES, not lines — one entry is a whole scan-outcome block of roughly 20 rendered lines — and the owner-approved value is 20, oldest-first eviction unchanged; not yet applied, queued behind D58 step 3 (in flight) to avoid two agents building the same module at once, so the shipped constant remains 200 until it lands. **Q43** opens a UI/UX enhancement, deferred under D57's freeze and not designed here: collapsible log entries, collapsed by default with a human-readable header line and the technical detail behind a toggle, settled at the module that owns the scanner's log UI rather than during the ownership refactor. Recording D59/Q43 is not new scope (NO-GO #10 intact). §10 (D59 added), §11 (Q43 opened), §15 annotated. |
| v1.39 — 2026-09-02, owner-approved | 2026-09-02 | D58 step 2 (amends item in D58's execution order, original D58 text kept) — the Pane cluster, second of D58's four steps, LANDED in commit `0d4daf7`: a new pure class, `PaneState`, becomes the sole owner of the pane decision's two inputs (tab index, `readInProgress`); `MainActivity.showPane()` no longer reads `tabLayout.selectedTabPosition` as an input, instead driving the `TabLayout`'s selection FROM `PaneState` (behind a re-entry guard, `applyingPaneStateToTabLayout`, so a programmatic move is never misread by the tab listener as a fresh user tap) — closing findings.md #1 (the rotation race) by construction: the framework's own, independently-timed tab-position restore (inside its default, still-un-overridden `onPostCreate`) can no longer race the pane decision, because the pane decision no longer consults it. `readInProgress` moves into the same owner — both its writers (`startSession`, `ReadTask.onPostExecute`) were already read-lifecycle-only, per this step's required survey — and stays deliberately non-persisted, matching its pre-existing reset-on-recreation behaviour. `HandoffAdmission`'s `av://` guard (findings.md #10's mitigation) is updated mechanically to read `paneState.readInProgress` in place of the old bare field, semantics unchanged. `PaneState` is a sibling of `PaneVisibility`, not an addition to it (the latter is a stateless singleton `object`; mutable fields on it would be shared across the two `MainActivity` instances that briefly coexist during a recreation), and exposes plain `Int`/`Boolean` rather than a `Bundle`, for the same reason D58 step 1's `ReportLog.restore` does (`Bundle.putInt`/`getInt` are non-functional stubs under this module's `isReturnDefaultValues = true`). Verified: unit tests 151→162 (JUnit XML, 0 failures, 11 new `PaneStateTest` cases covering all four legal states and both restore paths, failing-first demonstrated as 30 unresolved-reference compile errors before the production change); device-confirmed on the Pixel 6a (pid 17066, 2026-09-02) across four cases built on a real `av://` handoff and chip read — Log-tab-then-recreate, Scan-tab-then-recreate, Log-reselect-then-recreate, and two back-to-back recreations on Scan (the last added by the orchestrator, not in the original brief) — all four correct, tab state verified programmatically via a filtered `uiautomator dump` (SCAN/LOG `selected` attribute only) rather than a screenshot, because the scan form renders real MRZ fields. Findings.md #4's open question (whether the lock guard behaves correctly post-rotation) was traced by the coder as a report-only deliverable, no fix: the real guard (`lockModeAndArm`) never reads `lastMrzHash` at all, so it behaves correctly after a rotation by omission of that check, not by design; `lastMrzHash` is consulted in exactly one place, feeding only a value-free `Log.i` diagnostic, never a gate — the consequence is a logcat mislabel only ("first attempt" reported for a genuinely later attempt), with no effect on gating, security, or user-facing state. Finding #4 is narrowed, NOT closed — the diagnostic mislabel and the finding's other ten lost fields remain open, deferred to a later step. Findings.md #6 is unaffected by this step's field relocation — its named guard (the NFC-tag branch of `handleIncomingIntent`) still never consults `readInProgress` under either name; finding unchanged, still OPEN. Steps 3 (lock-time snapshot) and 4 (re-derived Session boundary) are not started. Full evidence in `docs/logs/M2-D58-STEP2-EVIDENCE.md`. §10 (D58's status line updated in place, no renumbering, no new decisions), §15 are annotated. |
| v1.38 — 2026-09-02, owner-approved | 2026-09-02 | D58 step 1 (amends item in D58's execution order, original D58 text kept) — the Report/Log cluster, smallest and lowest-consequence of D58's four steps, LANDED in commit `c856f42`: `ReportLog` absorbs `MainActivity.lastReportText` and is now the sole owner of the report/log cluster's state; `ReportLog.append` and the new `ReportLog.restore(saved, lastText)` are its only writers; `MainActivity.emitReport` and its new named sibling `MainActivity.restoreReport` are thin renderers over that owner, closing the doc/code mismatch findings.md #7 named by construction. `ReportLog.entries` gains a named `MAX_ENTRIES = 200` constant with oldest-first eviction, closing findings.md #13 FIXED-PROVISIONAL — the number is explicitly PROPOSED pending owner approval (sized from `ReportLogTest`'s rendered-fixture byte estimates against the ~1MB Bundle/binder transaction ceiling, not measured on a real device Bundle). Verified: unit tests 145→151 (JUnit XML, 0 failures, 6 new `ReportLogTest` cases, failing-first demonstrated as 13 compile errors before the production change); device-confirmed on the Pixel 6a (pid 15939, 2026-09-02) across a forced config-change recreation (auto-rotate is off on this device, so the orchestrator toggled `font_scale` to force it) — the restore path (`M2 stage: restored report/log across Activity recreation`) fired twice, once per recreation, with all three prior scan entries intact both times. Findings.md #5 (unfenced async `Thread{}` sites) is noted as partially touched — a late landing on the bounded structure is now harmless to this cluster's worst case, but the underlying no-cancellation defect is untouched and remains OPEN. Steps 2 (Pane cluster), 3 (lock-time snapshot), and 4 (re-derived Session boundary) are not started. Full evidence in `docs/logs/M2-D58-STEP1-EVIDENCE.md`. §10 (D58's status line updated in place, no renumbering, no new decisions), §15 are annotated. |
