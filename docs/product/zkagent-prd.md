...
- **Q23 (blocks M1 core, raised 2026-08-29 on M1 POC evidence) — tier A and attestation are in
  ...
  identifies the device. Options on the table, none chosen: (1) tier A drops device attestation
  and relies on something weaker; (2) attestation is verified by a party that never sees the
  holder's presentation (splits the verifier, reintroduces a live dependency); (3) Play Integrity's
  Google-signed token instead of the raw chain (must be captured and diffed the same way before it
  counts; excludes de-Googled devices, risk #7); (4) tier A's anonymity set is redefined as "this
  device" and the claim is withdrawn. Decide on evidence, not argument; M1b's method applies to
