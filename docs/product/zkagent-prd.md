...
| **M1 — Verifier SDK core** | 1 | `verdict.js`-style never-throw classifier; challenge nonce (port 8een `challenge.js`) and D20 signed challenges; tier negotiation (D19); trust-list check (FR10); attestation verification against chains captured from the M0 device (**POC done 2026-08-29, `docs/logs/M1-POC-EVIDENCE.md`**). Masterlist/SOD verification is the **phone's** job (the SOD never leaves the device); the verifier trusts the attested client. | Full negative matrix runs (bad attestation, replayed nonce, untrusted client, attestation root unreachable ⇒ `ok:false`; expired or non-pinned attestation root ⇒ `ok:true, allowed:false`); every reject-test paired with a non-vacuity pass-test |
...
4. **Attestation verification is implementable within our dependency rules** — whichever root Q14 selects. **M1 POC 2026-08-29: holds for parsing and chain verification — stdlib only, 163-line DER walker, zero deps.**
...
- **Q23 (blocks M1 core, raised 2026-08-29 on M1 POC evidence) — tier A and attestation are in
