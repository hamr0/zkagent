# zkagent — PRD v1.43 — 2026-09-02, owner-approved
...
**Version history**: §15. This revision (v1.43 — 2026-09-02, owner-approved) records **`.claude/remember/findings.md` #5 (zero async-cancellation discipline) FIXED, device-proven** — a `LifecycleFence`
(per-Activity-instance, retired in a new `onDestroy`, 11 sites fenced) closes D57's exit criterion (2).
**The freeze does NOT lift** — criterion (3) is not met, findings #10/#11 remain open-but-mitigated.
Device evidence (Pixel 6a, six tests) also answers Q38 (log lifetime: survives recreation, not process
death) and opens two new questions, Q47 (input-focus steal corrupting MRZ entry) and Q48 (three
indistinguishable reader apps on the launcher — a UX, not security, gap). New finding #16 (mint-report
loss after Activity destruction) is OPEN, unfixed by decision — a disclosure/product question, not a
code defect. Full evidence in `docs/logs/M2-FENCE-EVIDENCE.md`. §10 (D57 annotated), §11 (Q38
annotated, Q47/Q48 opened), §15 annotated. See below for the prior revision's own summary.
