# zkagent — PRD v1.44 — 2026-09-02, owner-approved
...
**Version history**: §15. This revision (v1.44 — 2026-09-02, owner-approved) is a **correction** to
v1.43's finding #5 record, following a `/branch-review` pass that found a real gap in that pass's
completeness claim: `BiometricPrompt.AuthenticationCallback` was missed because v1.43's "11 sites"
enumeration was syntactic (`runOnUiThread` text matches), not by the actual hazard predicate. Fixed
in commit `72e0b2c` — **13 fenced sites now, not 11**. **Stated precisely: D57's exit criterion (2)
was NOT actually met between commits `b8e0e05` and `72e0b2c`; it IS met now, from `72e0b2c`
onward.** The freeze still does NOT lift — criterion (3) remains unmet. No device evidence exists
for this gap or its fix, code/bytecode-verified only. Full record in
`.claude/remember/findings.md` #5's appended status-change note and
`docs/logs/M2-FENCE-EVIDENCE.md`'s follow-up section. §10 (D57 annotated in place), §15 annotated.
No new questions opened, no decisions invented, no renumbering. See below for the prior revision's
own summary.
