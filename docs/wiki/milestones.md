...
| No scanner changes (item 13) | Demo works against the released scanner APK unmodified | **No longer true, and flagged as a real exit-criterion gap, not papered over.** The M3 evidence gathered since 2026-09-05 (S1/S2/S3, D79) ran against a debug build carrying §6.5 S1–S3 and the D79 fixes (commits `d50d5de`, `aec100e`, `1bb86c8`, `a8df362`, `aca234b`, `def7b64`), not the released scanner APK unmodified as item 13 requires. The `apps/demo` server itself also gained a test-only `?threshold=` query parameter (S1's negative-test override) that does not exist in any released build. The released `chiproof`/scanner `v0.5.0` is **NOT** what the M3 evidence above was gathered against. This is a genuine conflict with item 13's own MUST NOT (§6.3 item 13: "no scanner-side changes required for M3... flagged back to the owner before work continues") — §6.5 was scoped as a scanner-side follow-up track explicitly outside M3 (§6.5's own header: "not in M3"), but the M3 evidence has in practice been collected against the §6.5 build, not the M3-clean one. Owner ruling needed: either (a) cut a new release incorporating §6.5 S1–S3/D79 and re-baseline M3's "released APK" to that, or (b) re-run the M3-specific exit rows above against the actual `v0.5.0` APK to confirm they still hold independent of the S1–S3/D79 changes. Not resolved by this pass. **Owner's answer, 2026-09-05: option (a)** — `v0.6.0` is being cut (CHANGELOG `[0.6.0]`) incorporating §6.5 S1–S3/D79, re-baselining M3's "released APK" to `v0.6.0`. This row is not marked passed by the release itself — cutting the build only gives M3 a released APK to re-run its exit rows against next; that re-run is unfinished work, tracked separately. |
